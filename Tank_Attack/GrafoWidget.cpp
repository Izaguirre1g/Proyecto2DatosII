...
void GrafoWidget::paintEvent(QPaintEvent *event) {
    ...
    // Dibujar obstáculos
    QPen obstaclePen(Qt::black);  // Color negro para los obstáculos
    QBrush obstacleBrush(Qt::black, Qt::SolidPattern);
    ...
    for (int i = 0; i < grafo->obtenerNumNodos(); ++i) {
        if (grafo->nodosBloqueados[i]) {
            ...
            painter.drawEllipse(x - 10, y - 10, 20, 20);  // Dibuja un círculo negro en el nodo bloqueado
        }
    }
}
