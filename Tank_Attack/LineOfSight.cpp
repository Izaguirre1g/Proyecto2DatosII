...
bool LineOfSight::tieneLineaVista(int nodoInicial, int nodoFinal) {
    ...
    //Crea un arreglo estático para almacenar los puntos de la línea (máximo 1000 puntos)
    int linea[1000][2];

    //Traza la línea entre los puntos y obtener el número de puntos generados
    int puntos = trazarLinea(x1, y1, x2, y2, linea, 1000);

    //Verifica si hay obstáculos en la línea
...
int LineOfSight::trazarLinea(int x1, int y1, int x2, int y2, int(*linea)[2], int maxSize) {
    ...
    return i;  // Devuelve el número de puntos generados
}

// Verifica si hay obstáculos en los puntos de la línea generada
bool LineOfSight::hayObstaculosEnLinea(int linea[][2], int puntos) {
    for (int i = 0; i < puntos; ++i) {
        ...
        // Verifica si el nodo cercano está bloqueado
