    ...
    if (usarBFS) {  // Movimiento con BFS
        ...
        if (nodoObjetivo != -1) {
            ...
            if (grafo->nodosBloqueados[nodoObjetivo]) {
                std::cerr << "El nodo objetivo está bloqueado. No se puede mover el tanque azul." << std::endl;
                return;
            }
...
int TanqueAzul::obtenerPosicionAleatoriaEnRadio(int radio) {
    int nodoAleatorio;
    int intentos = 0;
    do {
...
void TanqueAzul::moverAleatoriamenteConValidacion() {
    ...
    if (nuevoNodo != -1 && nodoActual != nuevoNodo) {
        ...
        // Hace pausas para permitir que el movimiento aleatorio sea visible
...
void TanqueAzul::continuarMovimiento() {
...
bool TanqueAzul::avanzarLoMasLejosPosibleConLineaVista() {
...
bool TanqueAzul::detectarColision(int siguienteNodo) {
...
void TanqueAzul::finalizarMovimientoYTurno() {
...
void TanqueAzul::avanzarCaminoPaso() {
    if (indiceCamino < longitudCamino) {
        nodoActual = camino[indiceCamino];  // Mueve al siguiente nodo en el camino
        indiceCamino++;
    }
}
